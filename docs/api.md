...
| Function | Description | | :--- | :--- | | `get(key)` | Return the value stored under `key`.
Raises `ResourceNotFoundError` if the key does not exist. | | `set(key, value)` | Store `value`
under `key`, overwriting any existing value. Returns the stored value. | | `delete(key)` | Remove
...
| Exception | Raised when | | :--- | :--- | | `ResourceNotFoundError` | The requested key does not
exist. | | `ServiceUnavailableError` | The database file could not be opened and loaded. |
